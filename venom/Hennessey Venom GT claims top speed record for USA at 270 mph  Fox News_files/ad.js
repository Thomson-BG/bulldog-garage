(function($) {
    $.ad = {
		...
		dc: {
			...
			pre: function(){
				window.get_aamCookie = function(c_name)
				{
		    ...
		    tag: function(data, tile, id, divElm) {
				...
				tagStr = valJoin([
					...
					"url=" + url,
					this.sid(true) + this.reginfo(), "sz=" + idd[1].replace(/_/g,",").replace("300x100","300x251,300x100").replace("300x250","300x250,300x600"), "tile=" + tile,
					"dc_ref="+ encodeURIComponent("http://www.foxnews.com"),
					window.get_aamCookie("fndfp"),
					"u="+get_aamCookie("aam_did"),
					"ord=" + $.ad.ord + "?"
				]);
		...
		gpt: {
			//window Orientation change or resize
			OnWindowResize : function(fn) {
				var hasOrientation = ("onorientationchange" in window) ? true : false,
					onEvent = (hasOrientation) ? "orientationchange" : "resize",
					timeout;

				$(window).bind(onEvent, function(){
	    	...
	    	getDartZone: function(d){
	    		...
	    		//if subsection5
	    		if(d.subsection5){
	    			zone.push(d.channel);zone.push(d.section);zone.push(d.subsection1);zone.push(d.subsection2);zone.push(d.subsection3);zone.push(d.subsection4);zone.push(d.subsection5);
	    			 return zone.join("/").replace(/-/g,"");
	    		}

	    		//if subsection4
	    		if(d.subsection4){
	    			zone.push(d.channel);zone.push(d.section);zone.push(d.subsection1);zone.push(d.subsection2);zone.push(d.subsection3);zone.push(d.subsection4);
	    			 return zone.join("/").replace(/-/g,"");
	    		}

	    		//if subsection3
	    		if(d.subsection3){
	    			zone.push(d.channel);zone.push(d.section);zone.push(d.subsection1);zone.push(d.subsection2);zone.push(d.subsection3);
	    			 return zone.join("/").replace(/-/g,"");
	    		}

	    		//if subsection2
	    		if(d.subsection2){
	    			zone.push(d.channel);zone.push(d.section);zone.push(d.subsection1);zone.push(d.subsection2);
	    			 return zone.join("/").replace(/-/g,"");
	    		}

	    		//if subsection1
	    		if(d.subsection1){
	    			zone.push(d.channel);zone.push(d.section);zone.push(d.subsection1);
	    			 return zone.join("/").replace(/-/g,"");
	    		}

	    		//if section
	    		if(d.section){
	    			zone.push(d.channel);zone.push(d.section);
	    			return zone.join("/").replace(/-/g,"");
	    		}
	    	...
	    	getSize: function(id){
					var idArr = id.split("-")[1],idArr = idArr.split("_"), allIdArrs = [];

					for(var i=0; i < idArr.length; i++){
						var temp = [], sizeArr = idArr[i].split("x");
						temp[0] = Number(sizeArr[0]);
						temp[1] = Number(sizeArr[1]);
			...
			getURL: function(){
				...
				url = u.substr(1).split("/").join("_").split(".")[0].replace(/[^a-z0-9_-]/g,'');

				var cus="", url = url + this.getKw(window.location, true) + this.getRef(true) + cus + location.hash;
				if(url == ""){
					var loc = location.hostname.split(".");
					loc.pop(); loc.pop(); loc.push(location.hash);
					url = loc.join(".");
				}
	    	...
	    	getC: function(){
				var classification = $.ad.meta().classificationISA || "";
				if(classification.length < 1){return false;}
				var cArr = classification.split(","), arr = [];
				for (var i = 0; i < cArr.length; i++) {
					arr.push($.trim(cArr[i]));
				}
				return arr.join(",");
			},
	    	_adv: "",
	    	...
	    	inJect: function(elm,d){

	    		var self = this, isLoaded = false, id = elm.attr("id"), d = $.ad.meta(), adSize = self.getSize(id), adv = self.getAdv(window.location, true), pos = self.getPos(id);

				function gloader(isLoaded){
					if(typeof AamGpt.getCookie("fngpt") != "undefined" &&
					typeof AamGpt.getCookie("aam_did") != "undefined" ){

						googletag.cmd.push(function() {
							googletag.defineSlot(self._dart, adSize , id)
								.addService(googletag.pubads())
									.setTargeting("ctype", d.ctype)
									.setTargeting("ptype", d.ptype)
									.setTargeting("url", self.getURL())
									.setTargeting("adv", adv)
									.setTargeting("pos", pos)
									.setTargeting("c", self.getC())
									.setTargeting("aamId", AamGpt.getCookie("aam_did"))
									.setTargeting(AamGpt.getKey("fngpt"), AamGpt.getValues("fngpt"));
	    	...
	    	init: function(d){
	    		var self = this;
	    		self._dart = self.getDartZone(d);

	    		var viewportWidth = $(window).width();
				var viewportHeight = $(window).height();

				$('.ad.gpt').each(function(){
					var elm = $(this), elmWrapper = elm.closest('.ad-container'), elmId = elm.attr('id');

		    		if(elmWrapper.css("display") != "none"){
		    			self.inJect(elm,d);
		    		}else{
		    			self._offAds.push(elmId)
		    		}

	    		});

		    	self.OnWindowResize(function(){
		    		for(var i = 0; i <= self._offAds.length-1; i++){
		    			$('#'+self._offAds[i]).each(function(){
		    				var elm = $(this), elmWrapper = elm.closest('.ad-container'), elmId = elm.attr('id');
		    				if(elmWrapper.css("display") == "block"){
		    					elmWrapper.addClass("done");
		    				}
		    			});
		    		}
		    	})
	    ...
	    qu: {
			...
			init: function(data) {
				...
				$('.ad.qu').each(function(){
					...
					if($.ad.clickBooth.isCB(id)){return;}

					if($.ad.perfectMarket.isPM(id)){return;}

					if( $.ad.broadspring.isBS(id)){return;}

					if(	$.ad.adBlade.isAdBlade(id)){return;}

					if( id === "qu_channel_7"){return;}

					if($.ad.visMeasure.isVisMeasure(id)){return;}
		...
		adBlade: {
			sectMatch : {
				"world" : "http://web.adblade.com/impsc.php?cid=1291-1494141695&output=html",
				//"science" : "http://web.adblade.com/impsc.php?cid=2485-2856742058&output=html",
				"entertainment" : "http://web.adblade.com/impsc.php?cid=3279-2530390552&output=html",
				"politics" : "http://web.adblade.com/impsc.php?cid=3287-2529196539&output=html",
				"travel" : "http://web.adblade.com/impsc.php?cid=2486-3256173949&output=html",
				"fbn" : "http://web.adblade.com/impsc.php?cid=2157-2754594435&output=html"
			},
			isAdBlade : function(qu_id) {
				var self = this, section = $('meta[name="prism.section"]').attr("content"), FBNflag = ($.ad._meta.channel.indexOf("fbn") > -1)? "fbn" : false;
				if($.ad._meta.raw.channel == "fnl"){return false;}
				return (qu_id == "qu_story_1" && (self.sectMatch[section] || FBNflag)) ? true : false;
			},
			init : function() {
				if($('#qu_story_1').size() == 0 || $.ad._meta.raw.channel == "fnl"){return;}

				var section = $('meta[name="prism.section"]').attr("content"), self = this;

				if(self.sectMatch[section]){
					var ifr = $.ad.util.iframe.create(660, 250, "adBlader");
					ifr.src = self.sectMatch[section];
					$('#qu_story_1').append(ifr);
				}
				if($.ad._meta.channel.indexOf("fbn") > -1){
					var ifr = $.ad.util.iframe.create(660, 250, "adBlader");
					ifr.src = self.sectMatch["fbn"];
					$('#qu_story_1').append(ifr);
				}
			}
		},
		adBladeCU: {
			sectMatch : {
				"entertainment" : "http://web.adblade.com/impsc.php?cid=3990-1659520999&output=html",
				"politics" : "http://web.adblade.com/impsc.php?cid=3287-2529196539&output=html"
			},
			init : function() {
				if($('#cu_article_1').size() == 0){return;}
				...
				$('.ad.cu').each(function(){
					...
					if((cuId == "cu_article_1") && (self.sectMatch[section])){
		...
		broadspringCU : {
			init: function(){
				...
				var sParams = {
					//for 612x250
					cu_article_1: {
			            id: "ae2acbef-1de8-4c9c-89d4-9f051758f3cd",
			            d:  "Zm94bmV3cy5jb20=",
			            wid: "7935"
					},
					//for 300x250
					cu_article_3: {
					    id: "e1d204a8-5617-4632-a681-4c2fdc16d325",
					    d:  "Zm94bmV3cy5jb20=",
					    wid: "7936"
					}
				...
				$('.ad.cu').each(function(){
					var targetElm = $(this), elmId = targetElm.attr("id");

					if(targetElm.css('display') == "block" && sParams[elmId]){

						(function() {

							var params = sParams[elmId];
					    	...
					    	s.async = true;
							document.getElementById(elmId).appendChild(s);
					    })();
					}
		...
		yieldmo : {
			init: function(d){
				var self = this, section = $('meta[name="prism.section"]').attr("content");

				if(section != "politics" && section != "entertainment"){return;}

				var paramId = {
					politics : "ym_714941275132599855",
					entertainment : "ym_785298484647108554"
				}

				$('.ad.cu').each(function(){
					var thisElm = $(this), elmId = thisElm.attr("id");
					if(thisElm.css('display') == "block"){

						if(elmId == "cu_article_3" || elmId == "cu_article_4"){
							thisElm.append('<div id="'+ paramId[section] +'" class="ym">');
							self.display();
						}
					}
				});
		...
		broadspring : {
			isBS : function(qu_id){
				var section = $('meta[name="prism.section"]').attr("content"), sectionStr = "us|opinion|tech|health|science|sports|leisure";
				return (qu_id == "qu_story_1" && (sectionStr.indexOf(section) > -1 /*|| $.ad._meta.channel.indexOf("fbn") > -1*/)) ? true : false;
			},
			init : function(){
				var self = this, section = $('meta[name="prism.section"]').attr("content"), p = {};

				if( $('#qu_story_1').size() > 0 && self.isBS("qu_story_1") ){
					if( $.ad._meta.channel.indexOf("fnc") > -1){
						var sectionParams = {
							us : {
	            			...
	            			politics : {
	            				id: "9b9f534c-736f-4c38-ba7e-4c92c5af1e3a",
								d: "Zm94bmV3cy5jb20=",
	            				wid: "5173"
	            			}
						};
						p = sectionParams[section];

					}else if($.ad._meta.channel.indexOf("fbn") > -1){

          				p = {
          					id: "c1970438-715c-4207-a5a1-192bed5dc946",
            				d: "Zm94YnVzaW5lc3MuY29t",
            				wid: "8246"
       	 				}
		...
		clickBooth: {
			isCB: function(id){
				return ($('#qu_story_1').length > 0 && ($.ad._meta.raw.channel == "fnl"))? true : false;
			},
			init: function(d){
				var self = this;
				if($('#qu_story_1').length > 0 && ($.ad._meta.raw.channel == "fnl")){
					self.display(d);
				}
			},
			display: function(d){
				...
				elm.empty();
   				...
   				src = "http://cbcpc.sitescout.com/disp?pid=2B7EB35479&rnd="+cacheBuster();
			    var attrs = {
			        "src": src, "width": "660", "height": "250", "scrolling": "no", "frameborder": "0", "border": "0", "marginwidth": "0",
			        "marginheight": "0", "hspace": "0", "vspace": "0", "id":"clickBooth"
			    };
		...
		clickBoothCU: {
			init: function(){
				var self = this, section = $('meta[name="prism.section"]').attr("content");
				if((section == "entertainment" || section == "politics") && $('meta[name="prism.channel"]').attr("content") == "fnc" && $('#cu_article_1').size() > 0){
					self.display();
				}
			},
			display: function(){
				...
				$('.ad.cu').each(function(){
					...
					if(targetElm.attr("id") == "cu_article_1" && targetElm.css('display') == "block"){

						var elm = $('#cu_article_1');
						var id = "cu_article_1";
						elm.empty();
		   				var ifr = document.createElement("iframe");
		   				src = "http://cbcpc.sitescout.com/disp?pid="+sectionPid[section]+"&rnd="+cacheBuster();
					    var attrs = {
					        "src": src, "width": "600", "height": "250", "scrolling": "no", "frameborder": "0", "border": "0", "marginwidth": "0",
					        "marginheight": "0", "hspace": "0", "vspace": "0", "id":"clickBoothCU"
					    };
						for (var attr in attrs) {
						    ifr.setAttribute(attr, attrs[attr]);
						}
		...
		perfectMarket: {
			//PM needed on all of FNC where sports, health, tech where #qu_story_4 is on the page
			//For FBN on HP #qu_channel_6 and everypage where #qu_story_4
			meta: function(){
				return {
					channel : $.ad._meta.raw.channel,
					section : $.ad._meta.raw.section,
					targetFNSections : "sports|health|tech|us|travel"
				}
			},
			isPM: function(id){
				var self = this, meta = self.meta();

				return ( (id == "qu_channel_6" && meta.channel == "fbn" && meta.section == "root") || (id == "qu_story_4" && (meta.channel == "fbn" || meta.targetFNSections.indexOf(meta.section) > -1))) ? true : false;
			},
			init: function(d){
				var self = this, meta = self.meta();

				if(meta.channel === "fnc" && $('#qu_story_4').length > 0){
					if( meta.targetFNSections.indexOf(meta.section) > -1){
						self.display(d);
					}
				}

				if(meta.channel === "fbn" )	{
					if(meta.section === "root" || $('#qu_story_4').length > 0){
						self.display(d);
					}
				}
			...
			display: function(d){
				var self = this, meta = self.meta(), elm, id, w, h;
				if( meta.channel == "fbn" && meta.section == "root"){
					elm = $('#qu_channel_6');
					id = "qu_channel_6";
					w = 156;
					h = 281;
				...
				elm.empty();
				//test
				var ifr = $.ad.util.iframe.create(w, h, id);
				ifr.setAttribute('id','ifr-perfectmarket');
				...
				if(meta.channel == "fbn"){
					doc.write("<html><head><style>body{margin:0px;}</style></head><body><script language=\"javascript\" type=\"text/javascript\"> var pm_adunit_id=1372466172; var pm_page_url = window.location.href; if (window!=window.top) { pm_page_url = parent.location.href;}</script><script type=\"text/javascript\" src=\"http://widget.perfectmarket.com/foxbusiness/load.js\"></script></body></html>");
				}
				if(meta.targetFNSections.indexOf(meta.section) > -1){
					doc.write("<html><head><style>body{margin:0px;}</style></head><body><script language=\"javascript\" type=\"text/javascript\"> var pm_adunit_id=1382474820; var pm_page_url = window.location.href; if (window!=window.top) { pm_page_url = parent.location.href;}</script><script type=\"text/javascript\" src=\"http://widget.perfectmarket.com/foxnews/load.js\"></script></body></html>");
				}
		...
		prsly: {
			pre: function(d){
				if(d.channel =="fnc/insider"){return;}

				var targetSites = "fnc/fbn/fsb/fnl", chan = d.channel.split("/")[0].toLowerCase();

				if(targetSites.indexOf(chan) == -1){
					return;
				}
			...
			insert: function(d,chan){
				...
				var siteName = ( d.channel == "fnc/nation")? "nation.foxnews.com" : sites[chan]; //updated
				...
				(function(s, p, d) {
					...
					$("body")
						.append('<div id="parsely-root" style="display: none">'+
							...
							'<div id="parsely-cfg" data-parsely-site="' + siteName + '"></div>'+
			...
			refresh: function(action_name){
				if(typeof PARSELY === "undefined"){return;}
				action_name = action_name || "SlideshowClick"
		...
		goog: {
			adsense: {
				...
				embed: function(obj) {
					var root = this;
					if(obj.qid == "front_7"){
						if($("#cu_"+obj.qid).css("display") == "block"){
			...
			googVidTrack : {
				init: function(){
					var self = this;
					if($('meta[name="prism.section"]').attr("content") == "video"){
						self.injectScript();
					}
				},
		...
		omni: {
			...
			init: function(d) {
				...
				(function(){
					var sDomains = ["Mapquest.com", "Pawnation.com", "everydayhealth.com", "mandatory.com", "theboot.com", "techcrunch.com", "Pawnation.com", "everydayhealth.com", "mandatory.com", "theboot.com",
					 ...
					 "huffingtonpost.com", "popeater.com", "kitchendaily.com", "travel.aol", "energy.aol", "slashfood.com", "gadling.com", "weirdnews.aol", "aolnews.com", "stylelist.com"];


					insertParam = function(linkElm,href){

						if(href.indexOf("txtlnkusaolp00000058") > -1){return;}
			...
			map: {
				...
				"trending":function(obj){window.omtr.linkTrackVars="eVar11,eVar12,eVar13,eVar14,eVar15,events";window.omtr.linkTrackEvents="event1";omtr.pageName = "fnc:social:trending:"+obj.section+":front:other";window.omtr.events="event1";window.omtr.t();},
		...
		track: function(d) {
			var track_map = {
				...
				"trending": function(obj){$.ad.omni.load({"trending": obj});},
		...
		luminate: {
			init: function(d){
				var self = this;
				if($.ad._meta.channel.indexOf("fnc") > -1 && d.ptype === "slideshow" && d.channel.indexOf("tech") > -1){
					self.injectPix();
				}
			},
			injectPix: function(){
				(function() {
				  ...
				  a.src = "http://www.luminate.com/widget/async/137fe39776b/";
		...
		visMeasure: {
			init: function(d){
				if(d.raw.channel == "fnl" && $('#qu_story_1').length == 1){
					this.display();
				}
			},
			isVisMeasure: function(id){

				return ($('meta[content="channel"]').attr("content") == "fnl" && id == "qu_story_1")? true : false;

			},
			display: function(){
				(function () {
				    var PLACEMENT = 26217, TARGET = "qu_story_1", targetElement = document.getElementById(TARGET),
				    rnd = Math.ceil(1000000 * Math.random()), u = '',r = '',view = '';
				    try {
				        u = encodeURIComponent(document.location);
				        r = encodeURIComponent(document.referrer);
				        view = encodeURIComponent(view);
				    } catch (e) {}
				    var src = "http://ad.viewablemedia.net/a/?pl=" + PLACEMENT + "&rnd=" + rnd + "&u=" + u + "&r=" + r + "&view=" + view;
				    ...
				    // empty the target of any children
		...
		visrev:{
			init: function(){
				...
				if($.ad._meta.raw.channel.indexOf("fnc") > -1){
					if($.ad._meta.raw.section == "root"){
						root.insertScript(115,true);
					}else{
						root.insertScript(115,false);
					}
				}
				if($.ad._meta.raw.channel.indexOf("fsb") > -1){
					if($.ad._meta.raw.section == "root"){
						root.insertScript(67,true);
					}else{
						root.insertScript(67,false);
		...
		gumgum: {
			init: function(d){
				var self = this;
				if(d.ptype != "slideshow" || typeof window.ISAManagementAds === "function"){return;}

				var lookUp = {
					"leisure" : "489afee2",
					"entertainment" : "0f459771",
					"tech" : "0310081f",
					"science" : "2f6a544b",
					"travel" : "ff7bad62"
				}
				if(lookUp[$('meta[name="prism.section"]').attr("content")]){
					window.ggv2id = lookUp[$('meta[name="prism.section"]').attr("content")];
					self.insertScript();
				}
			},
			insertScript: function(id,automate){
		...
		vibrantMedia: {
			init: function(d){
				var self = this, id = "50698";

				if(d.ptype != "slideshow" || typeof window.ISAManagementAds === "function"){ return false; }

				var section = $('meta[name="prism.section"]').attr("content") || false,
					allow = ["leisure", "entertainment", "tech", "science", "travel"];

				if (section && $.inArray(section, allow) > -1) {
					self.insertScript(id);
				}
			},
			insertScript: function(id){
				(function(d, a) {
					var s = d.createElement(a);
					s.async = true;
					s.src = 'http://foxnews.us.intellitxt.com/intellitxt/front.asp?ipid=50698';
					(d.getElementsByTagName("head") || d.getElementsByTagName("body"))[0].appendChild(s);
				})(document, 'script');
			}
		},
		tynt: {
		...
		nativo:{
			init: function(d){
				if(d.raw.channel.indexOf("fnc") > -1){
					(function(){
					    var script = document.createElement('script');
					    script.type = "text/javascript";
					    script.async = true;
					    script.id = "nativo";
						script.src = "http://a.postrelease.com/serve/load.js?async=true";
						setTimeout(function() { (document.getElementsByTagName("head")[0] || document.getElementsByTagName("body")[0]).appendChild(script) },1);
					}());
		...
		init: function() {
			if(window.adInitialized){return;}
			var d = $.ad.meta();
			var m = "server";
			...
			$.ad.clickBooth.init(d);
			$.ad.clickBoothCU.init(d);
			//tynt
			//$.ad.tynt.init(d);

			//$.ad.vibrantMedia.init(d);
			...
			$.ad.yieldmo.init(d);
			$.ad.goog.googVidTrack.init();
			//$.ad.broadspringCU.init();
			//$.ad.visMeasure.init(d);

			$.ad.perfectMarket.init(d);
